...
- `run_metadata.json`: 입력 파일, 실제 적용된 최적화 설정, 이상치 재배정 수, 주제 품질 지표 기록

CSV 결과는 UTF-8 BOM 형식으로 저장하므로 Excel에서 한글이 깨지지 않습니다.

## 주제 품질 지표

실행이 끝나면 요약에 네 가지 지표가 함께 출력되고, 같은 값이 `run_metadata.json`의 `coherence`에도 저장됩니다.

```
완료: 주제 32개, 이상치 문서 170개
품질 지표: c_npmi 0.1888 / u_mass -2.6800 / diversity 0.9375 (주제 32개 기준)
```

지표는 두 가지 질문에 답합니다. **앞의 두 개(coherence)는 "한 주제 안의 키워드들이 서로 어울리는가"**, **마지막 하나(diversity)는 "주제끼리 잘 구별되는가"**를 봅니다.

주제의 대표 키워드가 `치매, 돌봄, 요양, 가족`처럼 실제 문서에서 늘 같이 나오는 단어들이면 coherence가 높습니다. `치매, 로봇, 소설, 혈당`처럼 관계없는 단어가 섞이면 낮습니다. 그리고 주제 1번과 2번이 둘 다 `치매, 돌봄`을 대표 키워드로 쓰고 있으면 diversity가 낮습니다.

### 각 값의 뜻

| 지표 | 값의 범위 | 좋은 값 | 한 줄 설명 |
|---|---|---|---|
| `c_npmi` | -1 ~ 1 | **0.15 이상** | 키워드들이 문서에서 같이 등장하는 정도. **가장 먼저 볼 값** |
| `u_mass` | 음수 | 0에 가까울수록 | 같은 계열이지만 데이터마다 기준이 달라 순위 판단에는 쓰지 않음 |
| `diversity` | 0 ~ 1 | **0.7 이상** | 주제끼리 키워드가 겹치지 않는 정도 |

- `c_npmi`가 **0에 가까우면 우연히 같이 나온 수준**이라는 뜻입니다. 0.05 미만이면 주제가 제대로 안 잡혔다고 보고, 0.15를 넘으면 괜찮은 편, 0.3 이상이면 매우 잘 뭉친 주제입니다.
- `u_mass`는 음수가 정상입니다. `--reduce-frequent-words`처럼 흔한 단어를 걷어내는 옵션을 쓰면 결과가 좋아져도 이 값은 나빠지므로, 값이 낮아졌다고 실패한 것이 아닙니다.
- `diversity`가 1에 가까우면 주제마다 다른 단어를 쓰고 있다는 뜻입니다. 0.6 미만이면 사실상 같은 주제가 여러 개로 쪼개져 있는지 확인해야 합니다. 대표 키워드를 다듬는 `--keyword-diversity` 옵션과는 이름만 비슷하고 관계가 없습니다.

### 값을 보고 무엇을 할지

`c_npmi`와 `diversity`를 **항상 같이** 봅니다. 둘이 함께 올라갔으면 확실히 좋아진 것입니다.

| 이런 값이 나오면 | 이런 상태라는 뜻 | 이렇게 해봅니다 |
|---|---|---|
| `c_npmi` 높고 `diversity` 낮음 | 같은 주제가 여러 개로 쪼개짐 | `--reduce-topics auto`, `--min-topic-size` 올리기 |
| `diversity` 높고 `c_npmi` 낮음 | 드문 단어가 대표어가 되어 주제가 흐릿함 | `--min-word-frequency 3`, `--min-topic-size` 올리기 |
| 둘 다 낮음 | 불용어나 언어 설정 문제 | 불용어 파일 보강, `--language` 확인 |

주제 수와 이상치 수도 함께 봐야 합니다. 주제를 몇 개로 크게 묶으면 coherence는 저절로 올라가지만 결과가 지나치게 뭉뚱그려져 실제로 쓰기 어렵고, 이상치가 전체의 30%를 넘으면 지표가 좋아도 문서 상당수를 버린 상태입니다. 마지막 판단은 `topic_summary.csv`의 주제 이름을 직접 읽어서 하세요. 지표는 후보를 좁히는 도구입니다.

### 비교할 때 주의할 점

- **같은 데이터, 같은 전처리에서 설정만 바꿔 비교할 때** 의미가 있습니다. 불용어 파일을 바꾸거나 다른 데이터로 넘어가면 절대값이 함께 움직여 이전 값과 직접 비교할 수 없습니다.
- 문서 수가 적으면 값이 낮게 나옵니다. 같은 데이터에서 400건만 뽑았을 때 `c_npmi`가 0.1074, 6,349건 전체는 0.1888이었습니다. 샘플로 얻은 절대값을 전체 데이터의 기준으로 쓰지 마세요.
- 한국어 문서는 `치매를`, `치매가`처럼 조사가 붙은 채로 단어가 나뉘어 값이 실제보다 낮게 나옵니다. 한국어에서는 값의 **변화 방향만** 참고하세요.
- 이상치 주제(`-1`)는 주제가 아니므로 계산에서 제외합니다.
- coherence는 `치매`처럼 한 단어인 키워드만으로 계산합니다. `치매 돌봄`처럼 두 단어짜리 표현은 계산 방식상 위치가 왜곡되어 점수를 부당하게 낮추기 때문입니다. `diversity`는 이 영향이 없어 저장된 대표 키워드 전체로 계산합니다.
- 계산 시간은 문서 6천 건 기준 약 5초입니다. 문서가 훨씬 많아 부담이 되면 `--no-coherence`로 건너뛸 수 있습니다.
...
| `--coherence`, `--no-coherence` | 주제 품질 지표를 계산해 요약에 출력합니다. | 계산 | CLI 자체 기능 |
